...
namespace lemon {
	...
	public:
		...
		void install(std::size_t before_stack_off = 0, std::size_t after_stack_off = 0) {
			...
			// push original code
			patched_code_size_ = details::add_patched_code(code_, original_code_, original_size_, original_addr_);
		...
		void remove() {
			...
			if (op == 0xE8 && addr == rel_addr) {
				...
				code_->nop(128);
				// after
				code_->setSize(149 + patched_code_size_);
				code_->nop(124);
				code_->setSize(285 + patched_code_size_);
			}
